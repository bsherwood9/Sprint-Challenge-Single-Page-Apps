...
> Answer: Client-side routing comes with many benefits. Because we load in our whole application at the beginning, users don't have to wait for a request to be sent or information to be returned as they interact with the site. So it makes for a much smooter experience and users don't have to refresh the page. However, because we are loading the whole page on initial load, we load data that we may not even use or need, this may take more time.
